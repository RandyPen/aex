...
6. Otherwise: sign with `waap-cli sign-typed-data --data '<typed-data-json>' --json`. POST `{address, sig, data: {domain, types, message}}` to `${SNAPSHOT_SEQUENCER_URL}` (default `https://seq.snapshot.org`). The `data` field contains the full EIP-712 payload, not just the message. Return the Sequencer response.
