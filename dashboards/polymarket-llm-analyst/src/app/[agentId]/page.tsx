...
export default function AgentDashboard() {
  ...
  const { agent, balanceHistory, events, trades, positions, pnl, watchedMarkets, analyses, llmStats } = data;
  ...
  function getAgentHealthStatus(): { status: "trading" | "scanning" | "idle" | "stopped" } {
    if (agent.status === "stopped") return { status: "stopped" };
    for (const e of events) {
      if (e.type === "trade_filled" || e.type === "trade_placed") return { status: "trading" };
      if (e.type === "llm_analysis" || e.type === "market_scan") return { status: "scanning" };
    }
    return { status: agent.status === "running" ? "idle" : "stopped" };
  }
  ...
  return (
    <div className="container">
      ...
      <div className="grid grid-4" style={{ marginBottom: "var(--space-4xl)" }}>
        ...
        />
        <StatCard label="Analyses Run" value={llmStats.totalAnalyses.toString()} />
      </div>

      {/* 2. LLM Confidence chart + LLM Stats */}
        ...
        <div className="card">
          <div className="card-header">
            ...
            <span style={{ fontSize: "var(--font-size-text-xs)", color: "var(--color-text-40)" }}>
              Green = traded, Gray = skipped
            </span>
          </div>
          <ConfidenceChart analyses={analyses} confidenceThreshold={llmStats.confidenceThreshold} />
        </div>

        <LLMStats stats={llmStats} />
      </div>

      {/* 3. LLM Analysis History */}
      <div style={{ marginBottom: "var(--space-4xl)" }}>
        <AnalysisHistory analyses={analyses} confidenceThreshold={llmStats.confidenceThreshold} />
      </div>

      {/* 4. Balance trend + live activity */}
      <div className="grid grid-2" style={{ marginBottom: "var(--space-4xl)" }}>
        <div className="card">
          <div className="card-header">
      ...
      {/* 7. Market scanner */}
      <div style={{ marginBottom: "var(--space-4xl)" }}>
        <MarketScanner markets={watchedMarkets} />
      </div>
