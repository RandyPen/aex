    ...
    <div className="card">
      ...
      <div style={{
        ...
        <div style={{ color: "var(--color-text-60)" }}>
          The agent has executed {pnl.totalTrades} rebalances managing ${pnl.totalVolume.toLocaleString()} in swaps.
          {pnl.totalPnl >= 0 ? (
            <span style={{ color: "var(--color-green-50)" }}>
              {" "}Net positive with ${pnl.totalPnl.toFixed(2)} earned from threshold trading minus gas costs.
            </span>
          ) : (
            <span style={{ color: "var(--color-red-60)" }}>
              {" "}Currently net negative at -${Math.abs(pnl.totalPnl).toFixed(2)} (gas costs exceed trading gains).
            </span>
          )}
        </div>
      </div>
