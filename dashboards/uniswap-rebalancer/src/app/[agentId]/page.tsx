...
export default function AgentDashboard() {
  ...
  const { agent, balanceHistory, events, rebalances, position, fees, driftHistory, pnl } = data;
  ...
  function getAgentHealthStatus(): { status: "monitoring" | "rebalancing" | "idle" | "stopped" } {
    if (agent.status === "stopped") return { status: "stopped" };
    for (const e of events) {
      if (e.type === "rebalance_complete" || e.type === "rebalance_drained_no_remint") return { status: "rebalancing" };
      if (e.type === "range_check") return { status: "monitoring" };
    }
  ...
  return (
    <div className="container">
      ...
      <div className="grid grid-4" style={{ marginBottom: "var(--space-4xl)" }}>
        ...
        <StatCard
          label="Fees Earned"
          value={`$${fees.totalFeesUsd.toFixed(2)}`}
          color="var(--color-green-50)"
        />
        <StatCard
          label="Fee APY"
          value={`${fees.feeApyEstimate.toFixed(1)}%`}
          color="var(--color-green-50)"
        />
      </div>

      {/* 2. Pool position + portfolio balance */}
      <div className="grid grid-2" style={{ marginBottom: "var(--space-4xl)" }}>
        <PoolStatus position={position} />
        <PortfolioBalance position={position} />
      </div>

      {/* 3. Drift chart + recent activity */}
      <div className="grid grid-2" style={{ marginBottom: "var(--space-4xl)" }}>
        <div className="card">
          <div className="card-header">
            <span className="card-title">Price Drift from Range Center</span>
            <span style={{ fontSize: "var(--font-size-text-xs)", color: "var(--color-text-40)" }}>basis points</span>
          </div>
          <DriftChart data={driftHistory} rangeBps={500} />
        </div>
      ...
      {/* 4. Balance over time + fee earnings */}
      <div className="grid grid-2" style={{ marginBottom: "var(--space-4xl)" }}>
        <div className="card">
          <div className="card-header">
